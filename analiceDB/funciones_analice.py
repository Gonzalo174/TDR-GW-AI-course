...
Todo se calcula desde `DB/`. La lista de compuestos promiscuos, que exige datos
crudos, se calcula aparte (`datos_externos/promiscuidad/`) y aca sólo se mide su
...
def impacto_en_la_base(datos, promiscuos):
    """Cuánto saca el filtro de promiscuidad de la capa de subestructuras de `DB/`.

    La lista de promiscuos viene de afuera (`datos_externos/promiscuidad/`,
    calculada desde datos crudos); lo que sí se mide acá, desde la base, es su
    efecto sobre lo que el modelo usa: las filas de `sclus` de esas drogas y las
    aristas de `dds` que se quedan sin extremo. Aplica exactamente
    `tdr.filtrar_capa_quimica`, la función que usa `huerfanas/`.
    """
    filtrada = tdr.filtrar_capa_quimica(datos, promiscuos, verbose=False)
    presentes = int(datos.sclus["drug"].isin(set(np.asarray(promiscuos).tolist())).sum())
    return pd.DataFrame([{
        "compuestos_en_la_lista": len(promiscuos),
        "compuestos_presentes_en_la_base": presentes,
        "sclus_antes": len(datos.sclus), "sclus_despues": len(filtrada.sclus),
        "dds_antes": len(datos.dds), "dds_despues": len(filtrada.dds),
        "pct_dds_removido": 100 * (len(datos.dds) - len(filtrada.dds)) / len(datos.dds),
    }])
