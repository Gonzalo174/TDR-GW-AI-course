...
| `02_f04_grafo_anotaciones`<br>grafo_anotaciones() | `02_capas_y_conectividad.ipynb` | `02_conectividad_anotaciones.csv` | presentación | La misma tabla que `02_f03`, con su mismo alcance: sin control externo (`sin datos` en `10_equivalencia`). La diagonal de la matriz es el total de cada especie. |
| `02_f05_grafo_drogas`<br>grafo_drogas() | `02_capas_y_conectividad.ipynb` | `02_conectividad_drogas.csv` | presentación | La misma tabla que la matriz de drogas de `02`: sin control externo (`sin datos` en `10_equivalencia`). La diagonal de la matriz es el total de cada especie. |
...
| `03_f01_auc01_beta`<br>auc01_beta() | `03_comparacion_beta_promiscuidad.ipynb` | `03_resumen_beta.csv` | informe, presentación, página | Las AUC01 de beta=1 salen del mismo barrido que `02_optimos_por_especie` y `test_resultados` fija; la de beta=0 se recalcula con la misma metrica (`test_metricas`). |
...
| `03_f04_enriquecimiento_top100`<br>enriquecimiento_top100() | `03_comparacion_beta_promiscuidad.ipynb` | `03_resumen_beta.csv` | informe, presentación, página | El esperado es el que daría un ranking indiferente a ser quinasa: `top_k × n_quinasas / n_targets`, con las tres columnas en la misma tabla. |
...
| `01_f02_frank_por_grupo`<br>frank_por_grupo() | `01_pseudohuerfanas.ipynb` | `01_*_pseudohuerfanas.csv`<br>`01_k1_drugs.csv` | — | `test_fuga`: la orfanización quita todas las aristas de la droga. Contra el oráculo v4 cambia por el recorte y por la corrección de la semilla (PROVENANCE §3.16), que el oráculo no tiene. |
| `01_f03_semilla_por_especie`<br>semilla_por_especie() | `01_pseudohuerfanas.ipynb` | `01_*_pseudohuerfanas.csv` | informe, presentación, página | Consistencia interna: con semilla no informativa el puntaje del blanco es 0 por construcción, y se observa 0 % recuperado. Contra el oráculo v4 cambia por el recorte y por la corrección de la semilla (PROVENANCE §3.16), que el oráculo no tiene. |
...
| `03_f01_distribucion_rg`<br>distribucion_rg() | `03_desorfanizacion_global.ipynb` | `03_ranking_global.csv` | — | Contra el oráculo v4 (`03_ranking_global`): cambia por el recorte y por la corrección de la semilla (PROVENANCE §3.16), que el oráculo no tiene. |
| `03_f02_recuperacion`<br>recuperacion() | `03_desorfanizacion_global.ipynb` | `03_curva_recuperacion.csv`<br>`03_rg_estrella.csv` | informe, página | Contra el oráculo v4: r*G cambia por el recorte y por la corrección de la semilla (PROVENANCE §3.16), que el oráculo no tiene. No hay control independiente del corte; su sensibilidad a k_σ está en `03_rg_sensibilidad.csv`. |
| `03_f03_rss`<br>rss() | `03_desorfanizacion_global.ipynb` | `03_rss_distribucion.csv` | — | Contra el oráculo v4 (`03_rss_distribucion`): cambia por el recorte y por la corrección de la semilla (PROVENANCE §3.16), que el oráculo no tiene. |
...
| `03_f05_semilla_informativa`<br>semilla_informativa() | `03_desorfanizacion_global.ipynb` | `03_informativa.csv`<br>`03_informativa_topk.csv` | informe, presentación, página | Sin control independiente. Referencias internas: el azar en el panel (a); las dos clases se cuentan sobre la misma tabla que `03_f06`. |
| `03_f06_semilla_rss`<br>semilla_rss() | `03_desorfanizacion_global.ipynb` | `03_informativa.csv` | informe, presentación, página | Sin control independiente. El efecto del tamaño de semilla se comprobó dentro de cada organismo, no sólo en el total; el panel (b) muestra seis de los 16 organismos, elegidos por grupo de `tdr.META` y por cantidad de casos, no por el resultado. |
